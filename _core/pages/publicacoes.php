...
function saveForm($acao, $id){
    ...
    if($_FILES['img']['name'] != 0){
        ...
        if(!move_uploaded_file($_FILES['img']['tmp_name'], $config['DefaultPath'].'uploads/publicacoes/'.$nmImg)){
            $nmImg = "";
        }else{
            if($id > 0){
                ...
                if($row = mysqli_fetch_assoc($rs)){
                    unlink($config['DefaultPath'].'uploads/publicacoes/'.$row['img']);
                }
            }
        }
        $img = $nmImg;
    ...
    if($acao == 'add'){
        ...
        redirect('publicacoes', 'success-add');
    }else{
        tableUpdate($table, $fields, $id);
        redirect('publicacoes', 'success-edit');
    }
}
